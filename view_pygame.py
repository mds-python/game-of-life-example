...
class Display:
    ...
    def __init__(self, board, cmap):
        ...
        self._colors = [tuple(int(c * 255) for c in color[:3])
                       for color in (cmap(val / board.steps) for val in range(board.steps+1))]
    ...
    def animate(self, fps):
        ...
        while game_loop:
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_SPACE:
                        ...
                        dir = {pygame.K_LEFT: Board.LEFT, pygame.K_RIGHT: Board.RIGHT,
                               pygame.K_UP: Board.UP, pygame.K_DOWN: Board.DOWN}.get(event.key)
                        if dir is not None:
                            self.board.roll(dir)
                elif event.type == pygame.MOUSEBUTTONDOWN:
                    # Toggle cell on mouse click
                    pos = pygame.mouse.get_pos()
                    # `_getxy` method does not exits in this class, but must be present in the child classes
                    self.board.toggle(*self._getxy(pos))
            ...
            # Update the board accorgding to game logic
            if self.running:
                self.board.update()

            # Show changes
            # `_draw` method does not exits in this class, but must be present in the child classes
            self._draw()
            pygame.display.flip()
    ...
    def _draw(self):
        ...
        for y, row in enumerate(self.board.data):
            for x, val in enumerate(row):
                if val > 0:
                    pygame.draw.rect(self._screen, (self._colors[val]),
...
class Display3D(Display):
    ...
    This class implements pseude-3D display using PyGame engine.
    """
